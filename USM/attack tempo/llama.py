...
def parse_args():
    parser = argparse.ArgumentParser(description="TEMPO Attack: Llama Weight Column Matching")
    
    # Path Parameters
    parser.add_argument("--base_model", type=str, required=True, help="Base Llama model ID")
    parser.add_argument("--finetuned_path", type=str, required=True, help="Path to finetuned Llama")
    parser.add_argument("--cache_dir", type=str, default=None, help="Huggingface cache directory")
    
    # Extraction Parameters
    parser.add_argument("--layer_id", type=int, default=15, help="Llama layer index")
    parser.add_argument("--module_name", type=str, default="self_attn.o_proj", help="Sub-module path")
    parser.add_argument("--head_idx", type=int, default=0, help="Target head index")
    
    # Attack Hyperparameters
    parser.add_argument("--top_n", type=int, default=7, help="Number of candidates for voting")
    parser.add_argument("--dtype", type=str, default="float32", choices=["float32", "float64"])
    parser.add_argument("--device", type=str, default=None)

    return parser.parse_args()

def extract_llama_weight(model, args):
    """
    Extracts Llama weights. Transposes [out, in] to [in, out].
    """
    try:
        ...
        weight = target_module.weight.detach().clone().t()
        
        # Head slicing for attention modules
        if "self_attn" in args.module_name:
            num_heads = model.config.num_attention_heads
            head_dim = weight.shape[1] // num_heads
            return weight[:, args.head_idx * head_dim : (args.head_idx + 1) * head_dim]
        return weight
...
def main():
    ...
    # 1. Load Weights
    # For Llama, use float16 to load model to save memory, then cast specifically
    print(f"[*] Loading Llama models (fp16 for extraction)...")
    m_base = AutoModelForCausalLM.from_pretrained(args.base_model, cache_dir=args.cache_dir, torch_dtype=torch.float16).to(device)
    m_ft = AutoModelForCausalLM.from_pretrained(args.finetuned_path, cache_dir=args.cache_dir, torch_dtype=torch.float16).to(device)
    
    bs_w = extract_llama_weight(m_base, args).to(torch_dtype)
    ft_w = extract_llama_weight(m_ft, args).to(torch_dtype)
    ...
    # 3. Pre-compute Cache
    print("[Pre-compute] Caching ratios...")
    bs_ratio_cache = {}
    bs_harm_cache = {}
    ...
    print(f"\nLlama Final Accuracy: {hit}/{len(test_pairs)} ({hit/len(test_pairs):.2%})")
